...
public class NaviItem extends ListItem {
  ...
  @Getter private final List<NaviItem> subItems;
  protected Button expandCollapse;
  private int level = 0;
  private boolean subItemsVisible;

  public NaviItem(VaadinIcon vaadinIcon, String text, Class<? extends Component> navigationTarget) {
    this(text, navigationTarget);
    Icon icon = new Icon(vaadinIcon);
    icon.setSize("18px");
    icon.getStyle().set("margin-right", "15px");

    link.getElement().insertChild(0, icon.getElement());
  }
