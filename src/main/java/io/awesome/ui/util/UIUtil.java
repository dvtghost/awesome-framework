...
import io.awesome.ui.util.css.*;
import io.awesome.util.StringUtil;
...
public class UIUtil {
  ...
  public static void addTheme(String theme, Component... components) {
    for (Component component : components) {
      String existingTheme = component.getElement().getAttribute("theme");
      if (StringUtils.isNotBlank(existingTheme)) {
        component.getElement().setAttribute("theme", String.join(" ", existingTheme, theme));
      } else {
        component.getElement().setAttribute("theme", theme);
      }
    }
  ...
  public static HorizontalLayout createFormSectionTitle(String text) {
    ...
    layout.getStyle().set("background", "#293C58");
    Span span = new Span();
  ...
  public static VerticalLayout createMultiSelectLabelToStringComponent(String value, String width) {
    VerticalLayout componentLayout = new VerticalLayout();
    componentLayout.setPadding(false);
    componentLayout.setWidth(width);
    String truncate = StringUtil.getInstance().truncate(value, 100);
    Span span = new Span(truncate);
    componentLayout.add(span);
    return componentLayout;
  }

  public static Button createEditButton(String title) {
    Span buttonTitle = new Span(title);
    buttonTitle.getStyle().set("text-decoration", "underline");
  ...
  public static Notification createSuccessNotification(
      String message, int duration, Notification.Position position) {
    Notification notification = UIUtil.createSuccessNotification(message);
    notification.setDuration(duration);
    notification.setPosition(position);
    return notification;
  }

  public static Notification createErrorNotification(
      String message, int duration, Notification.Position position) {
    Notification notification = UIUtil.createErrorNotification(message);
    notification.setDuration(duration);
    notification.setPosition(position);
    return notification;
  }
