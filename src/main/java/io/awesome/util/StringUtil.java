...
public class StringUtil {
  ...
  public String generateCode(int length) {
    ...
    while (codeGen.length() < length) {
      codeGen.append(RandomStringUtils.randomAlphabetic(3));
      codeGen.insert(random.nextInt(codeGen.length()), RandomStringUtils.randomNumeric(1));
      codeGen.insert(random.nextInt(codeGen.length()), RandomStringUtils.randomNumeric(1));
      codeGen.insert(
          random.nextInt(codeGen.length()), RandomStringUtils.random(1, '#', '@', '$', '&', '?'));
    }
    return codeGen.substring(0, length);
  }
  ...
  public String removeString(String original, String toRemove) {
    if (original.contains(toRemove))
      return original.substring(0, original.indexOf(toRemove))
          + original.substring(original.indexOf(toRemove) + toRemove.length());
  ...
  public int[] getIndexes(String content, String token) {
    ...
    for (int i = 0; i < indexes.size(); i++) ret[i] = indexes.get(i).intValue();
  ...
  public String getLastToken(String string, String delim) {
    ...
    return string.substring(indexes[indexes.length - 1] + 1);
  }
  ...
  public String generateRandomToken(int gen_length, boolean encodeBase64) {
    ...
    if (encodeBase64) {
      ret = Base64Util.getInstance().encode(sb.toString().getBytes(StandardCharsets.UTF_8));
    }
    return ret;
  }
  ...
  public void errorFieldIsNotNumber(ErrorsDto errorsDto, String value, String fieldName) {
    if (StringUtils.isNotBlank(value)) {
      if (!StringUtils.isNumeric(value)) {
        errorsDto.add(new ErrorDto(fieldName, "This value should be a valid number."));
      }
    }
  }
}
