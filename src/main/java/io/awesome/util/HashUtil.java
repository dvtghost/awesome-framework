...
public class HashUtil {
  ...
  public String md5(String anyString, String salt) {
    try {
      // get UTF8 bytes
      byte[] queryStringToSignInBytes = anyString.getBytes(StandardCharsets.UTF_8);
      // create MD5 hash
      MessageDigest md5Digester = MessageDigest.getInstance("MD5");
      md5Digester.update(salt.getBytes(StandardCharsets.UTF_8));
      ...
      return Base64Util.getInstance().encode(hashedStringInHex.getBytes(StandardCharsets.UTF_8));
    } catch (Exception e) {
